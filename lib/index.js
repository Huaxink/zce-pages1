...
const html = () => {
    ...
    .pipe(swig(config.datas))
    .pipe(dest(config.temp))
    .pipe(bs.reload({ stream: true }))
}

const images = () => {
    return src(`${config.src}/assests/*`, { base: config.src })
    .pipe(image())
    .dest(dest(config.temp))
}

const extra = () => {
    return src([`${config.public}/*`, `!${config.public}/*.html`], { base: config.public })
    .dest(dest(config.temp))
}
...
const serve = () => {
    ...
    watch(`${config.public}/*.html`, html)
    
    // 对于不需要重新编译构建的文件，直接监听变化，然后用bs.reload刷新页面
    watch([`${config.src}/assests/*`, `${config.public}/*`, `!${config.public}/*.html`], bs.reload)
    bs.init({
        port: 1024,
        // files: 'dist/**', // 当用流进行 bs.reload时，不需要files了
        server: {
            baseDir: [config.dist, config.src, config.public]  // 服务是从左到右查找文件的
        }
    })
}
...
const build = series(clean, parallel(compile, images, extra), uesrefs)

const develop = series(compile, serve, uesrefs)
